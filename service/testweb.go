...
func (s *Server) SayhelloName(w http.ResponseWriter, r *http.Request) {
...
func (s *Server) Login(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		t1, err := template.ParseFiles("/Users/carlos/go/src/webtest/login.html")
		if err != nil {
			fmt.Println(err)
		}
		...
		if !CheckUsername(r.Form["username"][0]) || !CheckPassword(r.Form["password"][0]) {
		...
		if name == r.Form["username"][0] && psw == r.Form["password"][0] {
			...
			io.WriteString(w, "not formal user!")
		}
	}
}
...
func selInfo() (name, psw string, err error) {
	...
	checkErr(err)
	_selectInfoSQL := "SELECT `name`, psd from login WHERE id=2;"
	// 查询数据
